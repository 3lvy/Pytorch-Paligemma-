...
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        output = output * (1.o + self.weight.float())
...
class GemmaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = torch.gelu(y, approximate="tanh") # [Batch_Size, Seq_Len, Intermedaite_Size]
...
class GemmaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        self.dim = dim # it is set to the heawd_dim
        self.max_position_embeddings = max_position_embeddings
        self.base = base
...
class GemmaAttention():
    ...
    def forward(
        ...
        # [Batch_Size, Num_Heads_Q, Seq_Len, Head_Dim], [Batch_Size, Num_Heads_Q, Seq_Len, Head_Dim]
        ...
        value_states = repeat_kv(value_states, self.num_key_valule_groups)
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # Combine the embeddings of the image tokens, the tgext tokens and amsk out all the padding tokens
        final_embedding = torch.zeros(batch_size, sequence_length, embed_dim, dtype=inputs_embeds.dtype, device=inputs_embeds.device)
        # Shape: [Batch_Size, Seq_Len]. True for text tokens
        text_mask = (input_ids != self.config.image_token_index) & (input_ids != self.pad_token_id)
        # Shape: [Batch_Size, Seq_Len]. Trye for image tokens
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            ...
            if position_ids.dim() == 1:
                positon_ids = position_ids.unsqueeze(0)
        else:
